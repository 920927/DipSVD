...
def whitening(model_name, model, profiling_mat, ratio, dev):
    ...
    if 'opt' in model_name:
        ...
        layers = model.model.layers
    print("Start SVD decomposition after whitening...",len(layers))
    for i in tqdm(range(len(layers))):
        layer = layers[i]
        subset = find_layers(layer)
        #### Replace Attn, MLP ####
...
def whitening_dyn(y ,model_name, model_ori, profiling_mat, dev,calib_loader,ratio_sum):
    ...
    # if(np.all((x >= 0) & (x <= 1)))==False:
    #     print("Warining!!!!!!!!!!!!!!!!")
    #     print("Warning!!!!!   x inf:",x)
    #     print("Warining!!!!!!!!!!!!!!!!")
    #     # return float('inf')
    #     return 0.0001
    ...
    for i in tqdm(range(len(layers))):
        ...
        # # 计算网络参数
        # total = sum([param.nelement() for param in layer.parameters()])
        # # 精确地计算：1MB=1024KB=1048576字节
        # print(i,'layer: Number of parameter: % .4fM' % (total / 1e6))
    ...
    # torch.save({'model': model, 'tokenizer': tokenizer}, "/mnt/data2/zhuyao/SVD/vicuna_13b/tmp.pt")
    
    # del model
    # model, tokenizer = get_model_from_local("/mnt/data2/zhuyao/SVD/vicuna_13b/tmp.pt")
...
class local_update:
    def __init__(self, layer, scaling_diag_matrix, ratio, name, direct_update=False):
        ...
        # trucation SVD
        num_s_after_trunc = int(W.shape[0] * W.shape[1] * ratio / (W.shape[0] + W.shape[1]))
        self.truc_s = self.S[:num_s_after_trunc].to(torch.device("cuda:4"))
        self.truc_u = self.U[:, :num_s_after_trunc].to(torch.device("cuda:4"))
        if direct_update:
            self.truc_v = self.VT[:num_s_after_trunc, :].to(torch.device("cuda:4"))
        else:
            self.truc_v = torch.matmul(self.VT[:num_s_after_trunc, :].to(torch.device("cuda:4")), scaling_matrix_inv)
...
if __name__ == '__main__':
    ...
    if args.step == 1:
        ...
        ori_device = torch.device("cuda:4")
        torch.cuda.empty_cache()
        # model_ori = model_ori.half()
        ...
        # 定义域 32-7b;40-13b
        num_params = 40
        domain = [{'name': f'var_{i}', 'type': 'continuous', 'domain': (0.35, 0.95)} for i in range(num_params)]
        # # 约束
        # constraint = f"np.sum(x, axis=1) - {args.ratio * num_params}"
        # constraints = [{'name': 'sum_constraint', 'constraint': constraint}]
        ...
        # whitening_dyn(args.model, model, model_ori, profiling_mat, args.DEV)
        # model_name, model_ori, profiling_mat, dev,calib_loader
        # whitening_dyn(x ,model_name=args.model, model_ori=model_ori, profiling_mat=profiling_mat, dev=args.DEV,calib_loader=calib_loader)
       

        # 设置初始值
        initial_design_numdata = 1  # 初始设计点的数量
        # initial_X = np.full((initial_design_numdata, num_params), args.ratio)  # 初始值设置为均匀分布
        initial_X = np.array([[0.68278585,0.7600679,0.56314687,0.68367631,0.74932392,0.59732805,0.97081656,0.59930223,0.63419816,0.97081656,0.64756278,0.53404595,0.75796663,0.7460855,0.60703158,0.67044583,0.56348674,0.71978498,0.72146608,0.73490415,0.79591026,0.74504564,0.76879361,0.73673787,0.56469476,0.84871137,0.56761115,0.57980077,0.66579931,0.64190017,0.72411056,0.68595015,0.64867252,0.54692223,0.66747748,0.87083052,0.68366633,0.72960263,0.97081656,0.64270348]])
        ...
        # 创建优化器实例
        # myBopt = BayesianOptimization(f=objective_with_extra_param,
        #                             domain=domain,
        #                             acquisition_type='EI',
        #                             exact_feval=True,)
...
#         myBopt = pickle.load(f)
#     print("Optimizer state loaded.")
# except FileNotFoundError:
#     pass

# # 继续优化
# remaining_iter = max_iter - len(myBopt.Y)
# if remaining_iter > 0:
#     myBopt.run_optimization(max_iter=remaining_iter)
        ...
        if args.step == 5:
            ...
            ori_device = torch.device("cuda:2")
            model_ori = AutoModelForCausalLM.from_pretrained(args.model,device_map='cpu', trust_remote_code=True, cache_dir=None,torch_dtype="auto")
            print(model_ori.dtype)
            
            torch.cuda.empty_cache()
            model_ori.to(ori_device)
            # model_ori = deepspeed.init_inference(model_ori, mp_size=4)
