...
#         traindata = load_dataset("json", data_files="utils/c4-train.json")['train']
        traindata = load_dataset("json", data_files="utils/c4-train.00000-of-01024.json.gz")['train']

        tot_text = "\n\n".join(traindata["text"])
    elif name == "ptb":
#         traindata = load_dataset('ptb_text_only', 'penn_treebank', split='train', cache_dir=dataset_cache_dir)
#         tot_text = "\n\n".join(traindata["sentence"])
        traindata = load_dataset('/nfs/home/9303_xiechuanlong/dx/SVD/ptb_text_only', 'penn_treebank', split='train')
        tot_text = "\n\n".join(traindata["sentence"])
    elif name == "wikitext2":
#         traindata = load_dataset("wikitext", "wikitext-2-raw-v1", split="train", cache_dir=dataset_cache_dir)
        traindata = load_dataset("/nfs/home/9303_xiechuanlong/dx/SVD/wikitext-2-raw-v1", split="train", cache_dir=dataset_cache_dir)
#         tot_text = "\n\n".join(traindata["text"])
        tot_text = "\n\n".join(traindata["page"])
    else:
...
def get_calib_train_data_ppl(nsamples, tokenizer, seq_len=2048, batch_size = 4):
    ...
    test_dataset = process_data(test_data, tokenizer, seq_len, 'text')[:nsamples]
...
#     traindata = load_dataset('wikitext', 'wikitext-2-raw-v1', split='train', cache_dir=dataset_cache_dir)
    ...
    traindata = load_dataset("/nfs/home/9303_xiechuanlong/dx/SVD/wikitext-2-raw-v1", split="train", cache_dir=dataset_cache_dir)
    testdata = load_dataset("/nfs/home/9303_xiechuanlong/dx/SVD/wikitext-2-raw-v1", split="test", cache_dir=dataset_cache_dir)

#     trainenc = tokenizer("\n\n".join(traindata['text']), return_tensors='pt')
#     testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt')
    trainenc = tokenizer("\n\n".join(traindata['page']), return_tensors='pt')
...
#         test_data = load_dataset('wikitext', 'wikitext-2-raw-v1', split='test')
        test_data = load_dataset("/nfs/home/9303_xiechuanlong/dx/SVD/wikitext-2-raw-v1", split="test")
#         test_dataset = process_data(test_data, tokenizer, seq_len, 'text')
        test_dataset = process_data(test_data, tokenizer, seq_len, 'page')
    if 'ptb' in name:
#         test_data = load_dataset('ptb_text_only', 'penn_treebank', split='test')
        test_data = load_dataset('/nfs/home/9303_xiechuanlong/dx/SVD/ptb_text_only', 'penn_treebank', split='test')
