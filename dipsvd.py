...
@torch.no_grad()
def profle_dipsvd_low_resource(model_name, model, calib_loader, dev, channel_bar=0.05, channel_weight=30):
...
if __name__ == '__main__':
    ...
    if args.step == 1:
        ...
        profiling_mat = profle_dipsvd_low_resource(args.model, model, cali_white_data, args.DEV,channel_bar=0.03,channel_weight=30)
    ...
    ## 基于profile,贝叶斯搜索分配给每层的最佳ratio。
        ...
        if '7b' in args.model and ('llama' in args.model or 'vicuna' in args.model):
            num_params = 32
        ...
        elif '13b' in args.model and ('llama' in args.model or 'vicuna' in args.model):
            num_params = 40
            tokenizer = LlamaTokenizer.from_pretrained(args.model)
            print("num_params:",num_params) 
            batch_size_ = 1
